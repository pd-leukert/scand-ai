...
- `LLM_TIMEOUT` — seconds of silence from the model before the answering call gives up.
  ...
  the record and with the box: compose sets 1800, which is what a record at the default
  `LLM_NUM_CTX` costs on a laptop CPU (measured at about 17 tokens a second). On a timeout a
