...
| Contradiction alarm when a new document arrives | Medium — the reconciliation pass now exists (D42); needs ingesting a new document and reconciling it without redoing everything | Best demo moment in the list |
| Drift report: agreed vs. actually done | Medium — the supersedes and answers links now exist (D42) | Practice question P8 asks almost exactly this |
...
Two things D42 surfaces and does not decide. The topic summaries the reconciliation pass
...
paraphrase extraction writes for each statement, still is not in the answering context
(D28 flagged it as surface area nobody had decided to build on); this is the moment to
