...
## D48 — 2026-09-19 — Accepted *(merge note: reconciles this branch with D38–D41)*
**The answering prompt takes main's D40 and D41 as written — plain JSON under `STATEMENTS_JSON`,
the question first, compact separators — and this branch's record shape (D47) goes under that
label.**

PR #24 and this branch each moved the answering payload off base64 without knowing about the
other: main's D40 and this branch's D45 are the same change, and both were written the same day.
Neither supersedes the other; D45 also carries the size guard and the timeout numbers, D40 the
label. What was actually in conflict was smaller:

- **The label.** Main renamed `STATEMENTS_B64` to `STATEMENTS_JSON`; this branch had `STATEMENTS`.
  `STATEMENTS_JSON` wins, in both prompts, because it is already in a decision.
- **The order.** This branch put the record before the question, with a code comment saying so
  that Ollama would cache the shared prefix. Main's D41 decided the opposite, in the log. A code
  comment is not a decision, and a change that quietly contradicts one is the worse kind, so D41
  stands and the comment is gone.
- **The ids.** Main's D39 tells the model to copy long ids exactly. On the reconciled path the
  model is shown short aliases (D46, D47) and never a real id, so that instruction does not
  apply there and the prompt's example is `["s4", "s11"]`.
- **Numbering.** This branch's D40–D45 became D42–D47, and its D36a is dropped: main's D38 is the
  same entry under the number main gave it, and the two references to it now say D38.

*Cost:* D41 says nothing is lost by putting the question first because no prefix caching is
configured. Ollama does reuse a matching prompt prefix within a slot by default, and the record is
the long part of every prompt, so with the question first two different questions share only the
system prompt. On a CPU laptop at about 17 tokens a second (D45) that is the difference between
re-reading the record on every question and reading it once. Worth measuring before the demo;
if it matters, that is a new entry superseding D41, not an edit here.
